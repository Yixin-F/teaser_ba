...
void pointBA::optimize(const std::map<int, std::map<int, Eigen::Vector3f>>& covisibility,
                       ...
                       std::map<int, Eigen::Matrix4f>& opt_poses)
{
    ...
    auto iteration = covisibility.rbegin();
    ...
    for (auto asso_mp_mea = covisibility.begin(); asso_mp_mea !=  covisibility.end(); asso_mp_mea++) {
        ...
        for (auto obs_index = obs_group.begin(); obs_index != obs_group.end(); obs_index++) {
            ...
            Eigen::Vector3d mp_obs = obs_index->second;
    ...
    for (auto mp = covisibility.begin(); mp != covisibility.end(); mp++) {
        ...
        Eigen::Vector3f optimized_mappoint;
